...
function startValidation() {
  ...
  if (result.length == 0) {
    playMessage("Nenhuma palavra foi carregada.");
    alert("Nenhuma palavra foi carregada.");
  } else {
    stopPause = false;
    playMessage("Validação iniciada.");
          ...
          activeDiagram.commit((d) => {
            if (currentStep == result.length) {
              if (node.data.key == "r") {
                ...
                playMessage("Palavra aceita.");
              } else {
                d.model.set(node.data, "color", "red");
                playMessage("Palavra rejeitada.");
              }
            } else {
...
function cancelValidation() {
  ...
  setTimeout(function () {
    ...
    $("#word_container").empty();
    $("#word_container").append(
      '<h1 class="text-center">NÃO EXISTE PALAVRA ATIVA</h1>'
    );
